...
/**
 * NanoEditor — a faithful replica of GNU nano's interface.
 *
 * Keyboard behaviour matches real nano on Ubuntu:
 *   Ctrl+O        → "File Name to Write:" prompt; Enter confirms, Ctrl+C cancels
 *   Ctrl+X        → exit; if unsaved, asks "Save modified buffer?" Y / N / ^C
 *   Ctrl+K        → cut current line into cut-buffer
 *   Ctrl+U        → paste (uncut) cut-buffer at cursor
 *   Ctrl+W        → find (Where Is) — basic forward search
 *   Ctrl+G        → show brief help in status bar
 *   Ctrl+C        → show cursor position in status bar
 *   Ctrl+A / Home → beginning of line  (native)
 *   Ctrl+E / End  → end of line        (native)
 *   Ctrl+\        → replace            (alias for find+replace)
 *
 * Props
 *   path      string  — absolute VFS path of the file being edited
 *   initialText string — file content when the editor was opened
 *   onSave    (path, text) => void
 *   onClose   () => void
 *   onWriteMsg (msg) => void — optional; prints a message to the terminal after close
 */
export function NanoEditor({ path, initialText, onSave, onClose, onWriteMsg }) {
  ...
  return (
    <div
      className="absolute inset-0 z-50 flex flex-col font-mono text-[13px] leading-[1.4] rounded-xl overflow-hidden"
      style={{ background: "var(--term-bg)", color: "var(--term-fg)" }}
    >
      {/* ── Top title bar ── */}
      <div
        className="flex-shrink-0 text-center py-[2px] px-2 text-[13px] select-none"
      ...
      <textarea
        ...
        className="flex-1 min-h-0 resize-none outline-none px-2 py-1 text-[13px] leading-[1.4]"
      ...
      {/* ── Status / prompt row ── */}
      <div
        className="flex-shrink-0 min-h-[22px] px-2 py-[2px] text-[12px] select-none"
        style={{ background: "var(--titlebar)", color: "var(--titlebar-foreground)" }}
      >
        {mode === "exit-confirm" ? (
      ...
      {shortcuts.map((row, ri) => (
        ...
        >
          {row.map(([key, label]) => (
            <div key={key} className="flex items-center flex-1 min-w-0 border-r border-white/10 last:border-r-0">
              <span
                className="px-1 text-[11px] font-bold flex-shrink-0"
                style={{ background: "var(--term-fg)", color: "var(--term-bg)" }}
              >
                {key}
              </span>
              <span className="px-1 text-[11px] truncate" style={{ color: "var(--titlebar-foreground)" }}>
                {label}
              </span>
            </div>
          ))}
