import { HOME, modeString, shortDate, type FSNode } from "./fs";
import { evalArith, matchGlob, BreakSignal, ContinueSignal, ReturnSignal } from "./interpreter";
import { makeBinary, runScript } from "./exec";
import { checkC } from "./minic";
import type { CmdIO, CommandFn, ShellCtx } from "./types";

const abs = (ctx: ShellCtx, p: string) => ctx.fs.normalize(p, ctx.cwd);

function splitFlags(args: string[]): { flags: Set<string>; rest: string[]; opts: string[] } {
...
function inputText(io: CmdIO, ctx: ShellCtx, files: string[]): { text: string; error?: string } {
...
export const commands: Record<string, CommandFn> = {
  ...
  cd: (io, ctx) => {
    ...
    if (!ctx.fs.lookup(dest)) {
      io.err(`bash: cd: ${target}: No such file or directory\n`);
      return 1;
    }
    if (!ctx.fs.isDir(dest)) {
      io.err(`bash: cd: ${target}: Not a directory\n`);
      return 1;
    }
      ...
      if (flags.has("l")) {
        const body = entries
          .map((e) => {
            ...
            return `${modeString(e.node)} 1 student student ${String(size).padStart(6)} ${shortDate(
              e.node.mtime,
            )} ${e.name}`;
          })
          .join("\n");
  ...
  mkdir: (io, ctx) => {
    ...
    let status = 0;
    for (const t of rest) {
      const err = ctx.fs.mkdir(abs(ctx, t), flags.has("p"));
      if (err) {
        io.err(`mkdir: ${err}\n`);
        status = 1;
      }
  ...
  rmdir: (io, ctx) => {
    ...
    for (const t of io.args.slice(1)) {
      ...
      if (!node || node.type !== "dir") {
        io.err(`rmdir: failed to remove '${t}': Not a directory\n`);
        status = 1;
        continue;
      }
      if (Object.keys(node.children).length) {
        io.err(`rmdir: failed to remove '${t}': Directory not empty\n`);
        status = 1;
        continue;
      }
  ...
  rm: (io, ctx) => {
    ...
    for (const t of rest) {
      ...
      if (!node) {
        if (!flags.has("f")) {
          io.err(`rm: cannot remove '${t}': No such file or directory\n`);
          status = 1;
        }
        continue;
      }
      if (node.type === "dir" && !(flags.has("r") || flags.has("R"))) {
        io.err(`rm: cannot remove '${t}': Is a directory\n`);
        status = 1;
        continue;
      }
      ctx.fs.remove(path, true);
  ...
  touch: (io, ctx) => {
    for (const t of io.args.slice(1)) {
      ...
      if (node) node.mtime = Date.now();
        ...
        const err = ctx.fs.writeFile(path, "");
        if (err) {
          io.err(`touch: ${err}\n`);
          return 1;
        }
  ...
  cp: (io, ctx) => {
    const { flags, rest } = splitFlags(io.args.slice(1));
    if (rest.length < 2) {
      io.err("cp: missing file operand\n");
      return 1;
    }
  ...
  mv: (io, ctx) => {
    ...
    if (rest.length < 2) {
      io.err("mv: missing file operand\n");
      return 1;
    }
    const destRaw = rest[rest.length - 1]!;
    const destAbs = abs(ctx, destRaw);
  ...
  cat: (io, ctx) => {
    ...
    if (error) {
      io.err(`cat: ${error}: No such file or directory\n`);
      return 1;
    }
    if (flags.has("n")) {
      io.out(lines(text).map((l, i) => `${String(i + 1).padStart(6)}\t${l}`).join("\n") + "\n");
      return 0;
    }
  ...
  wc: (io, ctx) => {
    ...
    const { text, error } = inputText(io, ctx, rest);
    if (error) {
      io.err(`wc: ${error}: No such file or directory\n`);
      return 1;
    }
  ...
  grep: (io, ctx) => {
    ...
    if (pattern === undefined) {
      io.err("Usage: grep [OPTION]... PATTERN [FILE]...\n");
      return 2;
    }
    const re = new RegExp(pattern, flags.has("i") ? "i" : "");
    const multi = rest.length > 1;
    let matched = 0;
    const emit = (label: string, text: string) => {
      const ls = lines(text);
      let count = 0;
      ls.forEach((line, i) => {
        ...
        if (flags.has("v") ? !hit : hit) {
          ...
          matched++;
          if (!flags.has("c"))
            io.out(`${multi ? label + ":" : ""}${flags.has("n") ? i + 1 + ":" : ""}${line}\n`);
        }
      });
    ...
    if (!rest.length) emit("(standard input)", io.stdin);
      ...
      for (const f of rest) {
        const content = ctx.fs.readFile(abs(ctx, f));
        if (content === null) {
          io.err(`grep: ${f}: No such file or directory\n`);
          continue;
        }
        emit(f, content);
      }
  ...
  uniq: (io, ctx) => {
    ...
    while (i < ls.length) {
      ...
      out.push(flags.has("c") ? `${String(n).padStart(7)} ${ls[i]}` : ls[i]!);
      i += n;
    }
    io.out(out.join("\n") + (out.length ? "\n" : ""));
    return 0;
    ...
    io.out(
      lines(text)
        .map((line) => {
          ...
          return idx.map((i) => parts[i - 1] ?? "").join(delim);
        })
        .join("\n") + "\n",
    );
    return 0;
  ...
  tr: (io, ctx) => {
    ...
    const expand = (s: string) =>
      s.replace(/(\w)-(\w)/g, (_, a: string, b: string) => {
        let out = "";
    ...
    let text = io.stdin;
    if (flags.has("d")) {
      text = [...text].filter((c) => !from.includes(c)).join("");
    } else {
      text = [...text].map((c) => (from.includes(c) ? to[Math.min(from.indexOf(c), to.length - 1)] ?? c : c)).join("");
    }
    io.out(text);
    return 0;
  },
  sed: (io, ctx) => {
    ...
    const apply = (text: string) => {
      ...
      for (const script of scripts) {
        ...
        if (m) {
          out = out.replace(new RegExp(m[2]!, "m" + (m[4]!.includes("g") ? "g" : "") + (m[4]!.includes("i") ? "i" : "")), m[3]!);
          if (m[4]!.includes("g")) out = text.replace(new RegExp(m[2]!, "gm" + (m[4]!.includes("i") ? "i" : "")), m[3]!);
          continue;
        }
        const d = /^\/(.*)\/d$/.exec(script);
        if (d) out = lines(out).filter((l) => !new RegExp(d[1]!).test(l)).join("\n") + "\n";
      }
      return out;
    };
    if (!files.length) {
      io.out(apply(io.stdin));
      return 0;
    }
    for (const f of files) {
      const content = ctx.fs.readFile(abs(ctx, f));
      if (content === null) {
        io.err(`sed: can't read ${f}: No such file or directory\n`);
        return 1;
      }
  ...
  tee: (io, ctx) => {
    const { flags, rest } = splitFlags(io.args.slice(1));
    for (const f of rest) {
      const path = abs(ctx, f);
      if (flags.has("a")) ctx.fs.appendFile(path, io.stdin);
      else ctx.fs.writeFile(path, io.stdin);
    }
    io.out(io.stdin);
    return 0;
  },
  find: (io, ctx) => {
    ...
    for (let i = 0; i < args.length; i++) {
      if (args[i] === "-name") namePattern = args[++i] ?? null;
      if (args[i] === "-type") typeFilter = args[++i] ?? null;
    }
    const root = abs(ctx, start);
    if (!ctx.fs.lookup(root)) {
      io.err(`find: '${start}': No such file or directory\n`);
      return 1;
    }
  ...
  chmod: (io, ctx) => {
    ...
    if (!modeArg || !rest.length) {
      io.err("chmod: missing operand\n");
      return 1;
    }
    for (const f of rest) {
      const path = abs(ctx, f);
      const node = ctx.fs.lookup(path);
      if (!node) {
        io.err(`chmod: cannot access '${f}': No such file or directory\n`);
        return 1;
      }
      let mode = node.mode;
      if (/^[0-7]{3,4}$/.test(modeArg)) mode = parseInt(modeArg, 8);
        ...
        if (m) {
          ...
          if (!who.includes("a")) {
            let filter = 0;
            if (who.includes("u")) filter |= 0o700;
            if (who.includes("g")) filter |= 0o070;
            if (who.includes("o")) filter |= 0o007;
            mask &= filter;
          }
  ...
  stat: (io, ctx) => {
    for (const f of io.args.slice(1)) {
      ...
      const node = ctx.fs.lookup(path);
      if (!node) {
        io.err(`stat: cannot statx '${f}': No such file or directory\n`);
        return 1;
      }
      const size = node.type === "file" ? node.content.length : 4096;
      io.out(
        `  File: ${f}\n  Size: ${size}\t\tBlocks: 8\t IO Block: 4096   ${
          node.type === "dir" ? "directory" : "regular file"
  ...
  file: (io, ctx) => {
    for (const f of io.args.slice(1)) {
      ...
      if (!node) {
        io.out(`${f}: cannot open (No such file or directory)\n`);
        continue;
      }
      if (node.type === "dir") io.out(`${f}: directory\n`);
      else if (node.content.startsWith("\u007fELF"))
        io.out(`${f}: ELF 64-bit LSB pie executable, x86-64, dynamically linked\n`);
  ...
  ln: (io, ctx) => {
    const rest = io.args.slice(1).filter((a) => !a.startsWith("-"));
    if (rest.length < 2) {
      io.err("ln: missing file operand\n");
      return 1;
    }
  ...
  du: (io, ctx) => {
    ...
    let total = 0;
    for (const p of ctx.fs.walk(root)) {
      const n = ctx.fs.lookup(p);
      if (n?.type === "file") total += Math.ceil(n.content.length / 1024) || 1;
    }
  ...
  df: (io) => {
    io.out(
      [
        ...
        "browsercache     5242880  102400   5140480   2% /",
  ...
  free: (io) => {
    io.out(
      [
        ...
        "Mem:         8039412     2103244     4512180      102400     1424000     5620112",
  ...
  seq: (io) => {
    ...
    const [a = 1, b, c] = nums;
    const start = nums.length === 1 ? 1 : a;
    const step = nums.length === 3 ? (b as number) : 1;
    const end = nums.length === 1 ? a : nums.length === 3 ? (c as number) : (b as number);
    const out: string[] = [];
  ...
  expr: (io, ctx) => {
    const expr = io.args.slice(1).join(" ");
    const value = evalArith(expr.replace(/\\\*/g, "*"), ctx);
    io.out(value + "\n");
    return value === 0 ? 1 : 0;
  },
  bc: (io, ctx) => {
    const text = io.stdin || io.args.slice(1).join(" ");
    for (const line of lines(text)) {
      if (!line.trim()) continue;
      io.out(evalArith(line, ctx) + "\n");
    }
    return 0;
  },
  date: (io) => {
  ...
  id: (io) => {
    io.out("uid=1000(student) gid=1000(student) groups=1000(student),27(sudo)\n");
    return 0;
  },
  hostname: (io) => {
    io.out("ubuntu\n");
  ...
  uname: (io) => {
    ...
    if (flags.has("a"))
      io.out("Linux ubuntu 6.8.0-generic #1 SMP PREEMPT_DYNAMIC x86_64 x86_64 x86_64 GNU/Linux\n");
    else if (flags.has("r")) io.out("6.8.0-generic\n");
    else io.out("Linux\n");
    return 0;
  },
  uptime: (io) => {
    io.out(` ${new Date().toTimeString().slice(0, 8)} up 1:12,  1 user,  load average: 0.08, 0.03, 0.01\n`);
    return 0;
  },
  ps: (io) => {
    io.out(["    PID TTY          TIME CMD", "   1024 pts/0    00:00:00 bash", "   1099 pts/0    00:00:00 ps", ""].join("\n"));
    return 0;
  },
  top: (io) => {
    io.out("top: this build is non-interactive; use `ps` instead.\n");
    return 0;
  },
  clear: (_io, ctx) => {
    ctx.host.clear();
  ...
  sleep: async (io) => {
    const secs = Math.min(Number(io.args[1]) || 0, 10);
    await new Promise((r) => setTimeout(r, secs * 1000));
    return 0;
  },
  history: (io, ctx) => {
    ctx.history.forEach((h, i) => io.out(`${String(i + 1).padStart(5)}  ${h}\n`));
    return 0;
  },
  which: (io, ctx) => {
    let status = 0;
  ...
  man: (io) => {
    ...
    if (!name) {
      io.err("What manual page do you want?\n");
      return 1;
    }
  ...
  export: (io, ctx) => {
    for (const a of io.args.slice(1)) {
      const eq = a.indexOf("=");
      if (eq > 0) {
        ctx.vars[a.slice(0, eq)] = a.slice(eq + 1);
        ctx.exported.add(a.slice(0, eq));
      } else ctx.exported.add(a);
    }
    return 0;
  },
  unset: (io, ctx) => {
    for (const a of io.args.slice(1)) {
      delete ctx.vars[a];
      ctx.exported.delete(a);
    }
    return 0;
  },
  alias: (io, ctx) => {
    if (io.args.length === 1) {
      for (const [k, v] of Object.entries(ctx.aliases)) io.out(`alias ${k}='${v}'\n`);
      return 0;
    }
    for (const a of io.args.slice(1)) {
      const eq = a.indexOf("=");
      if (eq > 0) ctx.aliases[a.slice(0, eq)] = a.slice(eq + 1).replace(/^['"]|['"]$/g, "");
    }
    return 0;
  },
  unalias: (io, ctx) => {
    for (const a of io.args.slice(1)) delete ctx.aliases[a];
    return 0;
  },
  read: async (io, ctx) => {
    const args = io.args.slice(1);
    ...
    names.forEach((n, i) => {
      ctx.vars[n] = i === names.length - 1 ? parts.slice(i).join(" ") : parts[i] ?? "";
    });
    return 0;
  },
  ...
  basename: (io) => {
    const p = (io.args[1] ?? "").replace(/\/$/, "");
    let name = p.split("/").pop() ?? "";
    if (io.args[2] && name.endsWith(io.args[2])) name = name.slice(0, -io.args[2].length);
    io.out(name + "\n");
    return 0;
  },
  dirname: (io) => {
    ...
    io.out((idx <= 0 ? (idx === 0 ? "/" : ".") : p.slice(0, idx)) + "\n");
    return 0;
  },
  break: () => {
  ...
  return: (io) => {
    throw new ReturnSignal(Number(io.args[1]) || 0);
  },
  shift: (io, ctx) => {
    const n = Number(ctx.vars["#"] ?? 0);
    for (let i = 1; i < n; i++) ctx.vars[String(i)] = ctx.vars[String(i + 1)] ?? "";
    delete ctx.vars[String(n)];
    ctx.vars["#"] = String(Math.max(0, n - 1));
    return 0;
  },
  ...
  apt: (io) => {
    io.err(
      "apt: package management is not available in the cached shell.\nRun `ubuntu-vm` to boot the full Ubuntu machine where apt works.\n",
    );
    return 1;
  },
  "apt-get": (io) => {
    io.err("apt-get: not available here. Run `ubuntu-vm` for the full Ubuntu machine.\n");
    return 1;
  },
  python3: (io) => {
    io.err("python3: not installed in the cached shell. Run `ubuntu-vm` for the full Ubuntu machine.\n");
    return 127;
  },
  "ubuntu-vm": (io, ctx) => {
    io.out("Requesting the full Ubuntu virtual machine...\n");
    ctx.host.bootRealUbuntu();
    return 0;
  },
  reset: (_io, ctx) => {
    ctx.host.clear();
  ...
  "factory-reset": (io, ctx) => {
    ctx.fs.reset();
    ctx.cwd = HOME;
    io.out("Cached filesystem restored to defaults.\n");
    return 0;
  },
  help: (io) => {
...
function sourceFile(io: CmdIO, ctx: ShellCtx): Promise<number> | number {
  const target = io.args[1];
  if (!target) {
    io.err("source: filename argument required\n");
    return 2;
  }
  const content = ctx.fs.readFile(abs(ctx, target));
  if (content === null) {
    io.err(`bash: ${target}: No such file or directory\n`);
    return 1;
  }
  return runScript(content, { ...io, args: [target, ...io.args.slice(2)] }, ctx);
}

function runInterpreter(io: CmdIO, ctx: ShellCtx): Promise<number> | number {
  const args = io.args.slice(1).filter((a) => a !== "-x" && a !== "-e");
  if (args[0] === "-c") return runScript(args.slice(1).join(" "), { ...io, args: ["bash"] }, ctx);
  const target = args[0];
  if (!target) {
    io.err("bash: nested interactive shells are not supported here\n");
    return 1;
  }
  const content = ctx.fs.readFile(abs(ctx, target));
  if (content === null) {
...
function openEditor(io: CmdIO, ctx: ShellCtx, name: string): number {
  const target = io.args[1];
  if (!target) {
    io.err(`${name}: please give a filename, e.g. ${name} script.sh\n`);
    return 1;
  }
  const path = abs(ctx, target);
...
function compileC(io: CmdIO, ctx: ShellCtx, tool: string): number {
  const args = io.args.slice(1);
  let output = "a.out";
  const sources: string[] = [];
  for (let i = 0; i < args.length; i++) {
    const a = args[i]!;
    if (a === "-o") output = args[++i] ?? "a.out";
    else if (a.startsWith("-")) continue;
    else sources.push(a);
  }
  if (!sources.length) {
    io.err(`${tool}: fatal error: no input files\ncompilation terminated.\n`);
    return 1;
  }
  const parts: string[] = [];
  for (const src of sources) {
    const content = ctx.fs.readFile(abs(ctx, src));
    if (content === null) {
      io.err(`${tool}: error: ${src}: No such file or directory\n`);
      return 1;
    }
    parts.push(content);
  }
...
function runTest(args: string[], ctx: ShellCtx): number {
  ...
  if (args.length === 2) {
    ...
    switch (op) {
      ...
      case "-s":
        return ok(node?.type === "file" && node.content.length > 0);
      case "-r":
      case "-w":
        return ok(!!node);
      case "-x":
        return ok(!!node && (node.mode & 0o111) !== 0);
      case "-z":
        return ok(target === "");
      case "-n":
        return ok(target !== "");
  ...
  const [l, op, r] = args as [string, string, string];
  switch (op) {
    case "=":
    case "==":
      return ok(matchGlob(r, l) || l === r);
    case "!=":
      return ok(l !== r);
    case "-eq":
      return ok(Number(l) === Number(r));
    case "-ne":
      return ok(Number(l) !== Number(r));
    case "-lt":
      return ok(Number(l) < Number(r));
    ...
    case "-ge":
      return ok(Number(l) >= Number(r));
    case "-a":
      return ok(runTest([l], ctx) === 0 && runTest(args.slice(2), ctx) === 0);
    case "-o":
      return ok(runTest([l], ctx) === 0 || runTest(args.slice(2), ctx) === 0);
    case "<":
      return ok(l < r);
